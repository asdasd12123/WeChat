...
不管该堂课该学生被抽点了多少次　只要该学生请假成功　就不为缺勤
...
特例　请假已提交和请假已批准　
这两者优先级比前四种要高　一个学生只能向同一堂课请一次假　所以一旦出现优先级高的就会覆盖掉然后该学生的状态就被定下来了
...
初始化时若一个学生的任意一次考勤没有提交数据　那么就把他记为缺勤　其他置为null
若大于进行判断
若当前为空　状态直接修改为外部状态
若当前为正常或者迟到　外部除了缺勤任何一个状态只要优先级高能直接覆盖　若外部是缺勤　则修改为早退　
若当前为早退或缺勤　除了5,6能覆盖其他的都不变
若小于等于直接跳过
