...
判断老师或者学生存在就是直接判断当前的考勤队列是否存在老师或者学生
若都不存在则直接可以发起考勤
若老师或者学生有一个存在与考勤队列那么判断队列中的该元素是否已经下课
若没下课则不能踢出该课程　若下课则直接踢出
首先查看当前第一节是否已经下课 两个课程之间只能出现　
１上课　上课
２上课　下课　上课

所以除了第一个之外其他的只要存在就是上课　
因为计时器会踢掉已经到达时间的课程
当前的没有被踢掉说明时间不到正在进行或者是第一节末尾开始延续到了下节课
所以判断只需要判断第二个之后的矛不矛盾　若存在矛盾则无法进入
若都不矛盾　判断与第一个矛不矛盾　若矛盾则踢出第一个

若要判断一节课是否已经下课就需要参考时间和当前时间

若当前时间已经大于参考时间那么就是已经下课直接剔除

考勤队列和时间窗口原理
就是每进来一个课程就判断是否能进来进来的标准就是以上的结论
若能进来判断是否需要踢出当前的课程　若不存在则记录当前课程的开启时间和结束时间
若存在判断是否是第一个　若是则踢出该窗口关闭时间窗口重新计时　若不是则直接从队列中踢出该对象


时间窗口的目标就是不断的计时然后踢出第一个对象　在计算与下节课的时间再开始计时

所以考勤对象需要有个开始时间和结束时间 在下课时是不能开始考勤的

首先判断输入的信息合不合法
判断若前有没有考勤对象　若没有直接进入
判断当前的键是否和已有的重复　就是老师和学生是不是没有任何一个重复的　若没有则进入
若存在
