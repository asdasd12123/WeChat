...
class View(AuxiliaryFunction):
    ...
    def historical_statistics(self):   #这里的键值包括TeacherID和老师要统计的班级和次序号
        stuinfo = DataManage(DataManage.target_info, args=(self.detail_filename,)).run()
    ...
    def take_leave_absence(self, stuID):  # 请假认定
        ...
        while True:
            ...
            if num == 'yes':
                ...
                time.sleep(1)
                continue
        return DataManage(DataManage.update,args=(self.detail_filename, 'w', [stuinfo],
    ...
    def class_sum_info(self,key):
        ...
        classinfo = DataManage(DataManage.target_info, args=(filename,)).run()

        if not classinfo:
            print '该班级不存在考勤信息!'
            return False

        stulist = DataManage(DataManage.target_info, args=('../InData/studentInfo.csv',)).run()
    ...
    def get_all_rate(self,key):
        ...
        classinfo = DataManage(DataManage.target_info,args=('../InData/courseInfo.csv',
                                        {'TeacherID': key['TeacherID'], 'CourseName': key['CourseName']})).run()
            ...
            if seq['TeacherID'] == key["TeacherID"]:
                if seq['ClassID'] in classlist:
                    ...
                    if count == num:
                        ...
                        print '%-10s课第%2d次是对班级: %-10s发起,考勤的出勤率为%-3.2f %%' %(key['CourseName'],
                                                                    count, key['ClassID'],grade)
                        return grade
    ...
    def creat_sum(self, key, info=''):
        ...
        stulist = DataManage(DataManage.target_info, args=(self.detail_filename,)).run()
        stulist = self.statistics_calculation(stulist)
        olddata = DataManage(DataManage.target_info, args=(self.sum_filename,)).run()
        DataManage(DataManage.update, args=(self.sum_filename, 'dl', olddata)).run()
        ...
        return DataManage(DataManage.update, args=(self.sum_filename, 'w', olddata)).run()
    ...
    def init(self, key):
        ...
        if not DataManage(DataManage.target_key, args=(self.sum_filename,)).run():
            sum = []
            stulist = DataManage(DataManage.target_info, args=('../InData/studentInfo.csv',
