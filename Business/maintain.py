...
class maintain(object):
    ...
    def mainTain_stu(self): #手动维护学生信息
        classinfo=DataProcess(target=DataProcess.QueryObjectInfo,args=('../InData/courseInfo.csv',)).run()
        classlist=set()
        ...
        info=self.get_result('../InData/studentInfo.csv',info)
    ...
    def get_result(self,filename,data):

        sel=raw_input('是否修改学号?')
        if sel=='yes':
            stuID=raw_input('请输入学号!')
            if self.is_can(filename,{'StuID':stuID}):
                data['StuID']=stuID
            else:
                print '该学号不唯一无法修改成为此学号!'

        sel = raw_input('是否修改姓名?')
        if sel == 'yes':
            stuName = raw_input('请输入姓名!')
            data['StuName'] = stuName

        sel = raw_input('是否修改微信号?')
        if sel == 'yes':
            WeChatID = raw_input('请输入微信号!')
            if self.is_can(filename, {'WeChatID': WeChatID}):
                data['WeChatID'] = WeChatID
            else:
                print '该微信号不唯一无法修改成为此微信号!'

        sel = raw_input('是否修改个人信息特征?')
        if sel == 'yes':
            path = raw_input('请输入特征路径!')
            if self.is_can(filename, {'FeaturePath': path}):
    ...
    def mainTain_info(self):#手动维护考勤信息
        seqnum=raw_input('请输入您需要修改哪一次的记录！')
        filename='../InData/'+self.key['TeacherID']+'_'+self.key['ClassID']+'_'+seqnum+'_Detail.csv'
        if not DataProcess(target=DataProcess.QueryObjectKey,args=(filename,)).run():
            print '该次考勤不存在！'
            return False
        filename='../InData/'+self.key['TeacherID']+'_'+self.key['ClassID']+'_Sum.csv'
        stuinfo=DataProcess(target=DataProcess.QueryObjectInfo,args=(filename,)).run()

        print '该次学生考勤状态如下:'
        for stu in stuinfo:
            print stu['StuID'] +' : '+stu['check'+seqnum]
        ...
        print '该学生的该次的考勤状态 : %s' %(stu_info['check'+seqnum])
        stu_info['check'+seqnum]=raw_input('请输入你希望修改的值!')
